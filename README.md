...
- **前后端全链路**：REST API + 原生 SPA（口碑榜 / 发帖 / 规划 / 比价 / AI 助手 / 社区 / 运维 七模块）
...
`test_agent_react.mjs` 用 **fetch 桩把大模型换成可控脚本**，于是"模型要几轮工具、要哪个工具、何时收尾"都变成可断言的事件，而不是读代码猜：

| 断言 | 说明 |
|------|------|
| `tool_choice=auto` 确实发出 | 决策权真的交给模型，不是代码写死的分支 |
| 工具结果以 `role=tool` 回灌 | 回灌内容来自真实数据库（断言含 `¥` 报价），不是模型凭空编 |
| 模型不再要工具才收尾 | 验证"自终止"是模型驱动的，不是固定轮数 |
| 连续要工具时钉死在 4 轮 | 上限防死循环（防失控），超限给收尾话术而非报错 |
| 无 Key → 0 次模型请求 | 离线可跑；接口 500 → 降级规则兜底，管线不中断 |

**已知边界（不掩盖）**：这是对 agent 侧循环逻辑的验证；桩模拟的是"模型会如何要工具"，**不代表真实 DeepSeek 的决策质量**——真实模型的工具选择偏好需要接上 Key 才能观察。
...
├── public/            # 原生 SPA（口碑榜/发帖/规划/比价/AI助手/社区/运维）
